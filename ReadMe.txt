...
Ditton for CMCMTSREST.PY. For this micro app I was asked to repeatedly reset a modem using CLI to the terminal server. Basic metrics developed (fastest, slowest, average, total, # successful, etc.)
...
GRAPH_FILES.PY is included because I discussed it with Mr. Spero. It's a simple data visualization exercise, written to ingest Octoscope CVS files and output more useful data than what is obtainable from their onw GUI. Octoscope doesn't provide RSSI-vs-Throughput graphing and I thought it would be of overt benefit, so I rolled my own. It's a class only in the most literal sense of the word... very sloppy... but still demonstrable. I included a data set to run it against if you wish (RvRvO 5G US.csv) and the resultant plot (example.xlsx). The data originates from tests against unnamed 3rd-party (non-Altice) CPE.
